...
contract BNBBACK is ERC20, Ownable {
    ...
    address payable public marketingAddress =
        payable(0x2BB6D595B693731466C88F77D467eFFE8EF4b8eC);
    ...
    uint256 public totalBuyFees =
        marketingBuyFees.add(liquidityBuyFee).add(BNBRewardsBuyFee);
    uint256 public totalSellFees =
        marketingSellFees.add(liquiditySellFee).add(BNBRewardsSellFee);
    uint256 public totalTransferFees =
        marketingTransferFees.add(liquidityTransferFee).add(
            BNBRewardsTransferFee
        );
    ...
    */
    function updateBuyFees(
        ...
        totalBuyFees = marketingBuyFees.add(liquidityBuyFee).add(
            BNBRewardsBuyFee
        );
    ...
    */
    function updateSellFees(
        ...
        totalSellFees = marketingSellFees.add(liquiditySellFee).add(
            BNBRewardsSellFee
        );
    ...
    */
    function updateTransferFees(
        ...
        totalTransferFees = marketingTransferFees.add(liquidityTransferFee).add(
                BNBRewardsTransferFee
            );
        emit UpdateTransferFees(
            newMarketingFee,
            newLiquidityFee,
            newBNBRewardsFee
        );
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (
                isSelling && from != address(uniswapV2Router) // sells only by detecting transfer to automated market maker pair //router -> pair is removing liquidity which shouldn't have max
            ) {
                marketingFeeActual = marketingSellFees;
                liquidityFeeActual = liquiditySellFee;
                BNBRewardsFeeActual = BNBRewardsSellFee;
            } else if (isBuying && to != address(uniswapV2Router)) {
            ...
            uint256 marketingFeeAmount = amount.mul(marketingFeeActual).div(
                100
            );
            uint256 liquidityFeeAmount = amount.mul(liquidityFeeActual).div(
                100
            );
            uint256 BNBRewardsFeeAmount = amount.mul(BNBRewardsFeeActual).div(
                100
            );
    ...
    function swapTokensForBNB(uint256 tokenAmount, address payable account)
        private
    {
        if (tokenAmount <= 0) {
            return;
...
contract BNBBACKDividendTracker is DividendPayingToken, Ownable {
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "BNBBACK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBBACK contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BNBBACK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BNBBACK_Dividend_Tracker: Cannot update claimWait to same value"
        );
