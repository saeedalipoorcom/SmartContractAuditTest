...
interface IOCA {
    ...
    function multipleTransfer(address[] calldata recipients, uint256 amount)
        external;
...
contract OCA is IOCA, OrcaniaMath {
    ...
    constructor() {
        /// *********** CENT risk, we are set balance of owner to totalsupply;
    ...
    {
        require(
            (_balances[msg.sender] -= amount) < (uint256(-1) - amount),
    ...
    function transferFrom(
        ...
        require(
            (_balances[sender] -= amount) < (uint256(-1) - amount),
            "INSUFFICIENT_BALANCE"
        );
    ...
    function multipleTransfer(
        ...
        require(
            ...
            "MISMATCH_BETWEEN_RECIPIENT_AND_AMOUNT"
        );
        ...
        require(
            (_balances[msg.sender] -= total) < (uint256(-1) - total),
            "INSUFFICIENT_BALANCE"
        );
    }
    ...
    function multipleTransfer(address[] calldata recipient, uint256 amount)
        external
        override
    {
    ...
    function clearAllowance(address[] calldata users) external override {
        ...
        for (uint256 t; t < length; ++t) {
            _allowances[msg.sender][users[t]] = 0;
        }
    }
