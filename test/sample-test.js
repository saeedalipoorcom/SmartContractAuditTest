...
describe("Test Deploy", async function () {
  ...
  beforeEach(async function () {
    ...
    await MasterContract.setMasterDAOAddress(MasterDAOContract.address);
  ...
  describe("Bid On Current Auction , Mint NFT For Winner, Start New Auction", () => {
    ...
    it("", async () => {
      ...
      const blockNumber = await ethers.provider.getBlockNumber();
      getPriorVotes = await NFTContract.getPriorVotes(
        addr1.address,
        blockNumber - 1
      );
      console.log(
        "getPriorVotes of addr 1 after buy one nft",
        getPriorVotes.toString()
      );
      ...
      console.log(
        "getPriorVotes of master contract after transfer one nft",
        getPriorVotes.toString()
      );

      await MasterDAOContract.connect(addr1).propose(
        [owner.address],
      ...
      await MasterDAOContract.connect(addr1).castVote(1, 1);

      for (let i = 0; i < 10; i++) {
        ethers.provider.send("evm_mine");
      }

      const getReceipt = await MasterDAOContract.getReceipt(1, addr1.address);
      console.log("vote state ", getReceipt.toString());

      for (let i = 0; i < 90; i++) {
        ethers.provider.send("evm_mine");
      }

      const state = await MasterDAOContract.state(1);
      console.log("proposal state after 100 block", state.toString());

      TX = await MasterDAOContract.queue(1);
      await TX.wait(1);

      console.log((await ethers.provider.getBalance(owner.address)).toString());
      console.log(
        (await ethers.provider.getBalance(MasterDAOContract.address)).toString()
      );

      TX = await MasterDAOContract.execute(1);
      await TX.wait(1);
    });
  });
});
