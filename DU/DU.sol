...
contract DecentralizedUnited is Context, IERC20, Ownable {
    using Address for address;
    address payable public marketingWalletAddress =
        payable(0x2F38D7404b32aF52F8432C92b58c0F0d2BB3B145); // Marketing Wallet Address
    address payable public devWalletAddress =
        payable(0xCFBfc487f2e22a7E297B48623476F8ae1119C46a); // dev Wallet Address
    ...
    uint256 private _tTotal = 2 * 10**9 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "DecentralizedUnited";
    string private constant _symbol = "DCU";
    ...
    uint256 _saleMarketingFee = 9;

    uint256 public totalSwapableSaleFee =
        _saleTaxFee + _saleLiquidityFee + _saleDevFee + _saleMarketingFee;

    uint256 public _transferTaxFee = 0;
    uint256 public _transferLiquidityFee = 1;
    uint256 _transferDevFee = 0;
    uint256 _transferMarketingFee = 9;

    uint256 public totalSwapableTransferFee =
        _transferTaxFee +
    ...
    modifier isAuth() {
        ...
        require(
            owner() == msg.sender || msg.sender == safuDevAddress,
            "Auth: caller is not the owner or safuDev"
        );
        _;
    }

    constructor() {
        address _owner = 0xf75A59cA7CAaCa814B7462c50CaCC73F7310C89f;
    ...
    // مدیر میتونه هر شخصی رو از اینکه ریوارد دریافت کنه حذف کنه
    ...
    // مدیر میتونه مشخص کنه چ کسی ریوارد دریافت کنه
    ...
    // مدیر مشخص میکنه مکز چقدر پول در هر تراکنش نسبت به سوپلای میتونه منتقل بشه
    ...
    // مدیر میتونه مشخص کنه هر شخص مکز چقدر از والتش رو میتونه منتقل کنه
    ...
    // مدیر میتونه مشخص کنه چ کسی میتونه هر چقدر دلش خاست منتقل کنه
    ...
    function _transfer(
        ...
        if (
            ...
            //when sell
                ...
                to == uniswapV2Pair && !_isExcludedMaxTransactionAmount[from]
            ) {
                require(
                    amount <= maxTransactionAmount,
