    ...
    CoreV2
{
     ...
     */
    function initialize(uint256 ampFactor_, uint256 haircutRate_)
        external
        initializer
    {
        __Ownable_init();
    ...
    ///////////////////* centralaztion risk ! dev can pause contract anytimme *///////////////////
    function pause() external nonReentrant {
        _onlyDev();
        _pause();
    }

    ///////////////////* centralaztion risk ! dev can unpause contract anytimme *///////////////////
    ...
    /////////////////* centralaztion risk ! owner can set any fee *//////////////////
    function setFee(uint256 lpDividendRatio_, uint256 retentionRatio_)
        external
        onlyOwner
    {
        if (retentionRatio_ + lpDividendRatio_ > WAD)
     ...
     */
    /////////////////* centralaztion risk ! owner can address to any  *//////////////////
    ...
    /////////////////* why we used nonReentrant ? there is not externall call !  *//////////////////
    function addAsset(address token, address asset)
        external
        onlyOwner
        nonReentrant
    {
        ...
        )
    {
        ...
        lpTokenToMint = (
            liability == 0
                ? liabilityToMint
                : (liabilityToMint * asset.totalSupply()) / liability
        );
    }
    ...
    {
        IAsset asset = _assetOf(token);
        (liquidity, , reward) = _exactDepositToInEquil(
            asset,
            amount.toWad(asset.underlyingTokenDecimals())
        );
    }
    ...
    {
        // اول باید مقدار لیک رو داخل این است استخر بسوزونیم
        liabilityToBurn = (asset.liability() * liquidity) / asset.totalSupply();
        // چک میکنیم این مقدار صفر نباشه
    ...
    function _withdraw(
        ...
        // برداشت فی داره که باید حسابش کنیم
        _mintFee(asset);

        // calculate liabilityToBurn and Fee
        uint256 liabilityToBurn;
        // یوزر این مقدار میتونه برداشت کنه
        ...
        if (
            asset.liability() > 0 &&
            uint256(asset.cash()).wdiv(asset.liability()) < WAD / 100
    ...
    function withdraw(
        ...
        // مقدار رو از یوزر میگیریم
        IERC20(asset).safeTransferFrom(
            address(msg.sender),
            address(asset),
            liquidity
        );
        // دسیمال است رو در میاریم / برای ود نیاز داریم
        uint8 decimals = asset.underlyingTokenDecimals();
        // مقدار خروجی رو از برداشت میگیریم و از ود در میاریم
    ...
    function withdrawFromOtherAsset(
        ...
        // انتقال فرام از یوزر به آدرس فرام است
        ...
        // مقدار توووو به دست میاد / از ود در میاریم
        toAmount = toAmount.fromWad(toAsset.underlyingTokenDecimals());
        // انتقال میدیم به یوزر
        ...
        returns (uint256 amount, uint256 fee)
    {
        _checkLiquidity(liquidity);
        IAsset asset = _assetOf(token);
        // چقدر میشه از این استخر توکن برداشت کرد
        // چقدر میشه فی برای برداشت
        (amount, , fee) = _withdrawFrom(asset, liquidity);
        // از ود به عادی بر میگردونیم
    ...
    function _quoteFrom(
        ...
        // گرفتن مقدار خرجی ارز خروجی
        idealToAmount = _swapQuoteFunc(
            // فرام کش / کش ارز ورودی
            ...
            // مقدار ارز که کار بر میده
            fromAmount,
            int256(ampFactor)
        );

        // اگر کش ارز خروجی از مقدار ایده ال و حداقل خروجی کمتر شد ریورت میکنیم
        if (toCash < idealToAmount) revert WOMBAT_CASH_NOT_ENOUGH();

        // مقدار هیر کات رو میگیریم
        // این مقدار بر اساس ایدال مقدار خروجیه
        haircut = idealToAmount.wmul(haircutRate);
        // exact output swap quote has added haircut already
        if (fromAmount > 0) {
            // حالا مقدار خروجی میشه / ایدال خروجی منهای هیر کات / هیر کات به فی کالکت اضافه میشه
    ...
    function _swap(
        ...
        // گرفتن اکت مقدار و هیر کات از فانکشن کوووت فرم
        (actualToAmount, haircut) = _quoteFrom(
            fromAsset,
            toAsset,
            int256(fromAmount)
        );
        ...
        // اضافه کردن هیر کات به فی کالکت
        _feeCollected[toAsset] += haircut;

        // اضافه کردن مقدار ورودی به است ورودی به عنوان کش
        fromAsset.addCash(fromAmount);

        // حذف کردن مقدار خروجی از است خروجی
        // haircut is removed from cash to maintain r* = 1. It is distributed during _mintFee()
        toAsset.removeCash(actualToAmount + haircut);

        // revert if cov ratio < 1% to avoid precision error
        if (uint256(toAsset.cash()).wdiv(toAsset.liability()) < WAD / 100)
            revert WOMBAT_FORBIDDEN();
    }
    ...
    {
        // چک کردن مشابه نبودن آدرس ها
        _checkSameAddress(fromToken, toToken);
        // چک میکنیم مقدار صفر نباشه
        if (fromAmount == 0) revert WOMBAT_ZERO_AMOUNT();
        // چک کردن آدرس کسی که میخایم براش بفرستیم
        _checkAddress(to);
        // مطمعن شدن در مورد دد لاین
        _ensure(deadline);
        // چک میکنیم ارز استاپ نشده باشه
        requireAssetNotPaused(fromToken);
        ...
        // دسیمال ارز خروجی رو میگیریم
        uint8 toDecimal = toAsset.underlyingTokenDecimals();

        // مقدار واقعی خروجی و هیر کات رو میگیریم
        ...
        // در بالا ما به ود مقادیر رو دادیم و حالا به ود داریم پس باید از حالت ود در بیاریم
        // برای این کار باید از فانکشن و مقادیر دسیمال استفاده کنیم
        actualToAmount = actualToAmount.fromWad(toDecimal);
        haircut = haircut.fromWad(toDecimal);

        // بعد از پیدا کردن مقادیر واقعی / دریافت از کاربر و انتقال به کار بر رو انجام میدیم
    ...
    function quotePotentialSwap(
        ...
        // چک میکنیم که آدرس ها مشابه نباشن
        _checkSameAddress(fromToken, toToken);
        // مقدار ورودی رو چک میکنیم
        if (fromAmount == 0) revert WOMBAT_ZERO_AMOUNT();

        // ساختن اینترفیس از 2 توکن
        IAsset fromAsset = _assetOf(fromToken);
        IAsset toAsset = _assetOf(toToken);

        // exact output swap quote adds haircut
        // from amount is int256 !care
        if (fromAmount < 0) {
            fromAmount = fromAmount.wdiv(WAD_I - int256(haircutRate));
        }

        // now calc from amount ==> towad
        fromAmount = fromAmount.toWad(fromAsset.underlyingTokenDecimals());

        // get out
        (potentialOutcome, haircut) = _quoteFrom(
            fromAsset,
            toAsset,
            fromAmount
        );

        // out is now on wad / calc from wad
        potentialOutcome = potentialOutcome.fromWad(
            toAsset.underlyingTokenDecimals()
        );

        // cut is in wad / calc from wad
    ...
    function exchangeRate(address token) external view returns (uint256 xr) {
        ...
        return
            xr = uint256(asset.liability()).wdiv(uint256(asset.totalSupply()));
    }
        ...
        returns (uint256 equilCovRatio, uint256 invariant)
    {
        ...
        uint256 equilCovRatio = uint256(
            _equilCovRatio(invariant, SL, int256(ampFactor))
        );
        return (equilCovRatio, uint256(invariant));
    }

    function tipBucketBalance(address token)
        external
        view
        returns (uint256 balance)
    {
        IAsset asset = _assetOf(token);
        return
            asset.underlyingTokenBalance().toWad(
                asset.underlyingTokenDecimals()
            ) -
            asset.cash() -
            _feeCollected[asset];
    }
    ...
    /// ********* WHAI IF DEV DON'T DO THIS ************ ///
    function fillPool(address token, uint256 amount) external {
        _onlyDev();
        IAsset asset = _assetOf(token);
        uint256 tipBucketBal = asset.underlyingTokenBalance().toWad(
            asset.underlyingTokenDecimals()
        ) -
            asset.cash() -
            _feeCollected[asset];
    ...
    function _mintFee(IAsset asset) private {
        ...
        {
            // dividend to veWOM
            uint256 dividend = feeCollected.wmul(
                WAD - lpDividendRatio - retentionRatio
            );

            if (dividend > 0) {
                asset.transferUnderlyingToken(
                    feeTo,
                    dividend.fromWad(asset.underlyingTokenDecimals())
                );
            }
        }
        {
